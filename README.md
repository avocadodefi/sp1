...
SP1 is a performant, 100% open-source, and contributor-friendly zero-knowledge virtual machine (zkVM) that can prove the execution of arbitrary programs written in Rust or any other LLVM-compiled languages. SP1 democratizes access to ZKPs by allowing developers to use programmable truth with popular programming languages.
...
Today, developers can write programs, including complex, large programs like a ZK Tendermint light client, in Rust (with std support), generate proofs and verify them. Many Rust crates are supported and can be integrated seamlessly into your programs, though some may require adjustments for full compatibility. Example programs can be found in the [examples](https://github.com/succinctlabs/sp1/tree/main/examples) folder.
